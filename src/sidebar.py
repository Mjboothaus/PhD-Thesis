...
def create_sidebar(fluid):
    ...
    if fluid.symbol not in ["lj1", "lj2"]:
        psi_0 = st.sidebar.number_input(label=("\psi_0 (mV):"), value=0, min_value=-1000, max_value=1000)
    else:
        psi_0 = 0.0
    ...
    max_iteration = st.sidebar.number_input("Maximum iterations", min_value=1, max_value=1000, value=1)
